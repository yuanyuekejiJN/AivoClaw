...
export function setupAutoUpdater(): void {
  ...
  autoUpdater.on("update-downloaded", () => {
    ...
    log.info("[updater] preparing to quit and install");
...
export function checkForUpdates(manual = false): void {
  ...
  void autoUpdater.checkForUpdates().catch((err) => {
    log.error(`[updater] check for updates failed: ${formatUpdaterError(err)}`);
    if (manual) {
      void dialog.showMessageBox({
        type: "error",
        title: "Update Error",
...
export async function downloadAndInstallUpdate(): Promise<boolean> {
  ...
  downloadInFlight = autoUpdater
    ...
    .then(() => true)
    .catch((err) => {
      log.error(`[updater] download update failed: ${formatUpdaterError(err)}`);
      publishUpdateBannerState({ type: "download-failed" });
      return false;
    })
    .finally(() => {
