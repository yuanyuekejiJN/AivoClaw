...
type AivoClawPairingChannelState = {
  channel: string;
  pendingCount: number;
  requests: AivoClawPairingRequest[];
  updatedAt: number;
  lastAutoApprovedAt: number | null;
  lastAutoApprovedName: string | null;
};

type AivoClawIpcResult = {
  success?: boolean;
  message?: string;
};

type AivoClawPairingState = {
  pendingCount: number;
  requests: AivoClawPairingRequest[];
  updatedAt: number;
  channels: Record<string, AivoClawPairingChannelState>;
};

type AivoClawBridge = {
  onNavigate?: (cb: (payload: { view: "settings" }) => void) => (() => void) | void;
  onUpdateState?: (cb: (payload: AivoClawUpdateState) => void) => (() => void) | void;
  getUpdateState?: () => Promise<AivoClawUpdateState>;
  onPairingState?: (
    cb: (payload: AivoClawPairingState) => void,
  ) => (() => void) | void;
  getPairingState?: () => Promise<AivoClawPairingState>;
  refreshPairingState?: () => void;
  settingsApproveFeishuPairing?: (
    params: { code: string; id?: string; name?: string },
  ) => Promise<AivoClawIpcResult>;
...
export class OpenClawApp extends LitElement {
  static properties = {
    ...
    // 新布局状态
    leftSidebarTab: { state: true },
    rightSidebarOpen: { state: true },
    rightSidebarTab: { state: true },
    leftSidebarCollapsed: { state: true },
  };
  ...
  updateBannerState: AivoClawUpdateState = {
    status: "hidden",
    version: null,
    percent: null,
    showBadge: false,
  };
  pairingState: AivoClawPairingState = {
    pendingCount: 0,
    requests: [],
    updatedAt: Date.now(),
    channels: {},
  ...
  // 新布局状态
  leftSidebarTab: string = "agents";
  rightSidebarOpen: boolean = false;
  rightSidebarTab: string = "artifacts";
  ...
  private getAivoClawBridge(): AivoClawBridge | undefined {
    return (window as unknown as { aivoclaw?: AivoClawBridge }).aivoclaw;
  }

  // 规范化更新状态 payload，保证渲染层只消费合法值。
  private applyUpdateBannerState(payload: AivoClawUpdateState | null | undefined) {
  ...
  private applyPairingState(payload: AivoClawPairingState | null | undefined) {
    const rawRequests = Array.isArray(payload?.requests) ? payload.requests : [];
    const requests: AivoClawPairingRequest[] = rawRequests
      .map((item) => ({
    ...
    const channels = Object.fromEntries(
      Object.entries(rawChannels).map(([channel, item]) => {
        ...
        const normalizedRequests: AivoClawPairingRequest[] = channelRequests
          .map((request) => ({
            channel,
    ...
    ) as Record<string, AivoClawPairingChannelState>;
  ...
  private bindUpdateState() {
    ...
    const bridge = this.getAivoClawBridge();
  ...
  private getFirstPendingPairing(): AivoClawPairingRequest | null {
  ...
  private async fetchShareCopyPayload(): Promise<ShareCopyPayload | null> {
    const bridge = (window as unknown as {
      aivoclaw?: { settingsGetShareCopy?: () => Promise<unknown> };
    }).aivoclaw;
